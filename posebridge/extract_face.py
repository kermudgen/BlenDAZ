...
def create_face_camera(armature, camera_distance=0.6, ortho_scale=0.525, stage_coll=None, char_tag=None):
    """
    ...
    log.info(f"Created face camera parented to 'head' bone")
    log.info(f"  World position: {camera.location}")
    log.debug(f"  Head center: {head_world}")
    log.info(f"  Ortho scale: {ortho_scale}")
...
def get_face_control_positions(armature):
    ...
    head_bone = armature.data.bones.get('head')
    if not head_bone:
        log.warning("ERROR: 'head' bone not found")
        return {}
    ...
    log.info(f"Calculated {len(positions)} face control positions")
    for cp_id, pos in sorted(positions.items()):
        log.debug(f"  {cp_id}: ({pos.x:.4f}, {pos.y:.4f}, {pos.z:.4f})")
...
def generate_face_control_points(positions):
    ...
    log.info(f"Generated {len(control_points)} face control points")
...
def store_face_control_points(face_cps):
    ...
    log.info(f"Stored {len(face_cps)} face control points")
    return len(face_cps)
...
def setup_face_panel(armature=None, char_name=None, char_tag=None):
    ...
    if not armature or armature.type != 'ARMATURE':
        log.warning("ERROR: No armature found. Select a DAZ character first.")
        return None

    log.info(f"\n{'='*60}")
    log.info(f"Setting up Face Panel for: {armature.name}")
    log.info(f"{'='*60}")
    ...
    if char_name:
        ...
        log.info(f"  Collection: {stage_coll.name}")
    ...
    if not positions:
        log.warning("ERROR: Could not calculate face control positions")
        return None
    ...
    log.info(f"  Converted {len(positions)} positions to head-bone-local space")
    ...
    if missing:
        ...
        for prop in unique_missing:
            log.info(f"  - {prop}")
        log.info("These controls will have no effect until the properties are loaded.")

    # Step 5: Store control points
    cp_count = store_face_control_points(face_cps)

    log.info(f"\n{'='*60}")
    log.info(f"Face panel setup complete!")
    log.info(f"  Camera: {camera.name}")
    log.info(f"  Control points: {cp_count}")
    log.info(f"{'='*60}\n")
