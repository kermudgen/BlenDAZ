...
def create_genesis8_body_outline_simple(armature, outline_name="PB_Outline_Body"):
    ...
    if not armature or armature.type != 'ARMATURE':
        log.warning("Error: Need armature object")
        return None
    ...
    log.info(f"Arm/torso bones: {arm_bones}")
    ...
    try:
        ...
        log.warning(f"Error finding bones: {e}")
        log.warning("Check console for available bone names")
        return None
    ...
    log.info(f"Created simple outline: {outline_name}")
    return curve_obj
...
if __name__ == "__main__":
    ...
    if armature and armature.type == 'ARMATURE':
        log.info(f"Generating simple outline for: {armature.name}")
        create_genesis8_body_outline_simple(armature)
    else:
        log.info("Please select the Genesis 8 armature")
