...
def capture_fixed_control_points(armature, outline_name="PB_Outline_LineArt"):
    ...
    log.info(f"✓ Captured {count} fixed control point positions from T-pose")
    return count
...
def move_posebridge_setup(outline_name="PB_Outline_LineArt", offset_z=-30.0):
    ...
    if not camera:
        log.warning(f"❌ Camera '{camera_name}' not found.")
        return None, None
    ...
    if not outline:
        log.warning(f"❌ Outline '{outline_name}' not found.")
        return camera, None
...
def create_genesis8_lineart_outline(mesh_obj, outline_name="PB_Outline_LineArt", char_tag=None):
    ...
    log.info(f"\n{'='*60}")
    log.info(f"Starting Line Art Outline Generation")
    log.info(f"{'='*60}")
    ...
    if mesh_copy.data.shape_keys:
        ...
        log.info(f"  Stripped {sk_count} shape keys from mannequin")
    ...
    if removed_mods:
        log.info(f"  Removed {len(removed_mods)} modifiers: {', '.join(removed_mods[:5])}"
                 f"{'...' if len(removed_mods) > 5 else ''}")

    # STEP 2: Create temp collection and move copied mesh
    log.info("Step 2: Moving mesh to temp collection...")
    ...
    if temp_layer_coll:
        ...
        log.info(f"  Ensured temp collection is enabled: exclude={temp_layer_coll.exclude}, hide={temp_layer_coll.hide_viewport}")
    else:
        log.warning(f"  Warning: Could not find layer collection for temp collection")
    ...
    if hasattr(mesh_copy, 'visible_get'):
        log.info(f"  Mesh copy visible_get(): {mesh_copy.visible_get()}")

    log.info(f"  Moved to collection: {temp_collection.name}")
    log.info(f"  Collection visibility: viewport={not temp_collection.hide_viewport}, render={not temp_collection.hide_render}")
    if temp_layer_coll:
        log.info(f"  Layer collection: exclude={temp_layer_coll.exclude}, hide_viewport={temp_layer_coll.hide_viewport}")
    log.info(f"  Mesh copy visibility: viewport={not mesh_copy.hide_viewport}, hidden={not mesh_copy.hide_get()}, selectable={not mesh_copy.hide_select}")

    # STEP 3: Exclude original collection from view layer (CRUCIAL!)
    log.info("Step 3: Excluding original collection from view layer...")
    ...
    if original_collection:
        ...
        if layer_coll:
            ...
            log.info(f"  ✓ Excluded collection '{original_collection.name}' from view layer")
        else:
            log.warning(f"  ✗ Warning: Could not find layer collection for '{original_collection.name}'")
    ...
    log.debug(f"  Character height: {character_height:.2f}m")
    ...
    log.info("Step 4: Creating camera...")
    ...
    ortho_scale = 5.0  # Fixed scale — frames full body with padding
    camera_data.ortho_scale = ortho_scale
    log.debug(f"  ortho_scale: {ortho_scale} (fixed, width={character_width:.2f}m, height={character_height:.2f}m)")
    ...
    log.debug(f"  Camera at: {camera.location} (Z = {camera_z:.2f}m = 58% of height)")
    log.info(f"  Camera type: ORTHOGRAPHIC (ortho_scale = 5.0)")

    # STEP 5: Create light at same location, power 50
    log.info("Step 5: Creating light...")
    ...
    log.debug(f"  Calculated Line Radius: {line_radius:.4f}")
    ...
    if bpy.app.version >= (5, 0, 0):
        ...
        log.info("Blender 5.0 detected: Line Art will automatically use GP layers and materials")
    else:
        ...
        if hasattr(lineart_mod, 'target_layer'):
            try:
                ...
                log.warning(f"Note: Could not set target_layer: {e}")

        if hasattr(lineart_mod, 'target_material'):
            try:
                lineart_mod.target_material = gp_data.materials[mat_index]
            except RuntimeError as e:
                log.warning(f"Note: Could not set target_material: {e}")
    ...
    log.info(f"\n{'='*60}")
    log.info(f"=== Line Art Modifier Configuration ===")
    log.info(f"{'='*60}")
    log.info(f"\n🎬 Scene Settings:")
    log.info(f"  Render Engine: {bpy.context.scene.render.engine}")
    log.info(f"  Film Transparent: {bpy.context.scene.render.film_transparent}")
    log.info(f"\n📝 GP Object & Layers:")
    log.info(f"  GP Object: {gp_obj.name}")
    log.debug(f"  GP Data: {gp_data.name}")
    log.debug(f"  Layer: {gp_layer.name if gp_layer else 'None (auto-created)'}")
    log.info(f"  Material: {mat.name if mat else 'None (auto-created)'} (index: {mat_index})")
    log.info(f"\n⚙️ Line Art Modifier Settings:")
    log.info(f"  Type: {lineart_mod.type}")
    log.info(f"  Source Type: {lineart_mod.source_type}")
    log.info(f"  Source Object: {lineart_mod.source_object.name if lineart_mod.source_object else 'None'}")
    log.info(f"  Original Mesh: {mesh_obj.name}")
    log.info(f"  Mesh Copy: {mesh_copy.name}")
    log.info(f"  Excluded Collection: {original_collection.name if original_collection else 'None'}")
    log.info(f"  Temp Collection: {temp_collection.name}")
    log.info(f"  Custom Camera: {camera.name if lineart_mod.use_custom_camera else 'Default'}")
    log.info(f"  Camera Distance: 12 units")
    log.info(f"  Light: {light.name} ({light.data.energy}W)")
    if hasattr(lineart_mod, 'target_layer'):
        log.info(f"  Target Layer: {lineart_mod.target_layer if lineart_mod.target_layer else 'Auto'}")
    if hasattr(lineart_mod, 'target_material'):
        log.info(f"  Target Material: {lineart_mod.target_material.name if lineart_mod.target_material else 'Auto'}")
    log.info(f"\n📐 Edge Types:")
    log.debug(f"  Contour/Silhouette: {lineart_mod.use_contour if hasattr(lineart_mod, 'use_contour') else 'N/A'}")
    log.info(f"  Intersection: {lineart_mod.use_intersection if hasattr(lineart_mod, 'use_intersection') else 'N/A'}")
    log.info(f"  Loose: {lineart_mod.use_loose if hasattr(lineart_mod, 'use_loose') else 'N/A'}")
    log.info(f"  Crease: {lineart_mod.use_crease}")
    log.info(f"\n🔍 Filtering & Quality:")
    log.info(f"  Silhouette Filtering: {lineart_mod.silhouette_filtering if hasattr(lineart_mod, 'silhouette_filtering') else 'N/A'}")
    log.info(f"  Overscan: {lineart_mod.overscan if hasattr(lineart_mod, 'overscan') else 'N/A'}")
    log.info(f"  Level Start: {lineart_mod.level_start if hasattr(lineart_mod, 'level_start') else 'N/A'}")
    log.info(f"  Level End: {lineart_mod.level_end if hasattr(lineart_mod, 'level_end') else 'N/A'}")
    log.info(f"\n🎨 Geometry Processing:")
    log.info(f"  Backface Culling: {lineart_mod.use_back_face_culling if hasattr(lineart_mod, 'use_back_face_culling') else 'N/A'}")
    log.info(f"  Instancing: {lineart_mod.use_instancing if hasattr(lineart_mod, 'use_instancing') else 'N/A'}")
    ...
    log.info(f"    GP object visible: {not gp_obj.hide_viewport}")
    log.info(f"    Mesh copy visible: {not mesh_copy.hide_viewport}, hidden: {mesh_copy.hide_get()}")
    log.debug(f"    GP data layers: {len(gp_data.layers)}")
    if bpy.app.version >= (5, 0, 0) and hasattr(gp_data, 'drawings'):
        log.info(f"    GP drawings: {len(gp_data.drawings())}")
    ...
    if bpy.app.version >= (5, 0, 0):
        ...
        try:
            ...
            log.info(f"  ✓ Applied Line Art modifier")
        except RuntimeError as e:
            log.warning(f"  ✗ Warning: Could not apply modifier: {e}")
    else:
    ...
    if bpy.app.version >= (5, 0, 0) and hasattr(gp_data, 'drawings'):
        ...
        log.info(f"    GP drawings: {num_drawings}")
        if num_drawings > 0:
            drawing = gp_data.drawings()[0]
            log.info(f"    Curves in first drawing: {len(drawing.curves)}")
    log.info(f"    GP object scale: {gp_obj.scale}")

    # Flatten GP object (scale to 0 on Y-axis for front view)
    log.info("Flattening GP object (Y-axis scale = 0)...")
    gp_obj.scale[1] = 0.0
    log.info(f"  ✓ GP scaled to {gp_obj.scale}")

    # Convert mesh copy into a mannequin background for control points
    log.info("Setting up mesh copy as mannequin...")
    ...
    log.info(f"  ✓ Unlocked all transform channels on {mesh_copy.name}")

    # Strip all modifiers
    for mod in list(mesh_copy.modifiers):
        mesh_copy.modifiers.remove(mod)
    log.info(f"  ✓ Removed all modifiers from {mesh_copy.name}")

    # Strip all existing materials
    mesh_copy.data.materials.clear()
    log.info(f"  ✓ Cleared all materials from {mesh_copy.name}")
    ...
    log.info(f"  ✓ Applied '{mat_name}' material to {mesh_copy.name}")

    # Keep visible in viewport, hide from render
    mesh_copy.hide_viewport = False
    mesh_copy.hide_render = True
    log.info(f"  ✓ Mannequin visible in viewport: {mesh_copy.name}")
    ...
    log.info(f"  ✓ Moved '{mesh_copy.name}' → {stage_coll.name}")
    log.info(f"  ✓ Moved '{gp_obj.name}' → {stage_coll.name}")
    if temp_collection_name in bpy.data.collections:
        bpy.data.collections.remove(bpy.data.collections[temp_collection_name])
        log.info(f"  ✓ Deleted temp collection '{temp_collection_name}'")

    log.info(f"\n{'='*70}")
    log.info(f"✨ SUCCESS! Line Art Outline Created: {outline_name}")
    log.info(f"{'='*70}")
    log.info(f"\n🎯 Workflow Applied:")
    log.info(f"  1. Copied mesh: {mesh_obj.name} → {mesh_copy.name}")
    log.info(f"  2. Moved copy to temp collection: {temp_collection_name}")
    log.info(f"  3. Excluded original collection from view layer")
    log.debug(f"  4. Created camera: 12 units from character, Z = {camera_z:.2f}m (58% of height)")
    log.info(f"  5. Created light: Same position, 50W power")
    log.info(f"  6. Selected mesh copy and created GP with LINEART_OBJECT")
    log.info(f"  7. Applied Line Art modifier (while mesh visible)")
    log.info(f"  8. Flattened GP object (Y-axis scale = 0)")
    log.info(f"  9. Converted mesh copy to mannequin and re-enabled original collection")
    log.info(f"\n✅ Critical Settings:")
    log.info(f"  ✓ Render Engine: EEVEE")
    log.info(f"  ✓ Source: {mesh_copy.name} (copy, not original!)")
    log.info(f"  ✓ Original Collection: {original_collection.name if original_collection else 'N/A'} (temporarily excluded)")
    log.debug(f"  ✓ Character Height: {character_height:.2f}m")
    log.debug(f"  ✓ Line Radius: {line_radius:.4f} (dynamic)")
    log.info(f"  ✓ Silhouette Filtering: GROUP")
    log.info(f"  ✓ Edge Types: Silhouette ONLY")
    log.debug(f"  ✓ Camera: Y=-12 units, Z={camera_z:.2f}m (58% height)")
    log.info(f"  ✓ Light Power: 50W")
    log.info(f"  ✓ GP Flattened: Y-scale = 0.0")
    log.info(f"\n📦 Objects Created:")
    log.info(f"  • GP Object: {gp_obj.name}")
    log.info(f"  • Mesh Copy: {mesh_copy.name}")
    log.info(f"  • Temp Collection: {temp_collection_name} (deleted after use)")
    log.info(f"  • Camera: {camera.name}")
    log.info(f"  • Light: {light.name}")
    log.info(f"\n🎨 The outline is now static (modifier applied) and flattened for front view!")
    log.info(f"  • GP Object: {outline_name}")
    log.info(f"  • Position control points using bone locations from front camera perspective")
    log.info(f"  • Original mesh is still active - pose it to test control point placement")

    log.info(f"\n💡 Next Steps:")
    log.info(f"  1. Calculate control point positions from bone locations (front camera view)")
    log.info(f"  2. Use bone head/tail/midpoint positions projected to 2D viewport coords")
    log.info(f"  3. Place circle controls for single bones, sun controls for multi-bone groups")

    log.info(f"\n❓ If you don't see the outline:")
    log.info(f"  1. Check '{outline_name}' visibility in outliner")
    log.info(f"  2. Ensure viewport shading shows overlays (top right icons)")
    log.info(f"  3. Verify GP object scale is (1, 0, 1) - flattened on Y")
    log.info(f"  4. Check that Line Art modifier was applied successfully")
    log.info(f"{'='*70}\n")

    # CAPTURE FIXED CONTROL POINT POSITIONS from T-pose
    log.info(f"\n{'='*70}")
    log.info(f"Capturing Fixed Control Point Positions...")
    log.info(f"{'='*70}")
    if armature:
...
if __name__ == "__main__":
    ...
    if obj:
        ...
        if obj.type == 'MESH':
            mesh_obj = obj
            log.info(f"Generating Line Art outline for mesh: {obj.name}")
        elif obj.type == 'ARMATURE':
            # Find mesh that uses this armature
            for scene_obj in bpy.context.scene.objects:
                if scene_obj.type == 'MESH':
                    for mod in scene_obj.modifiers:
                        if mod.type == 'ARMATURE' and mod.object == obj:
                            mesh_obj = scene_obj
                            log.info(f"Generating Line Art outline for mesh: {mesh_obj.name} (from armature {obj.name})")
                            break
                if mesh_obj:
                    break
        ...
        if mesh_obj:
            ...
            log.info("No valid mesh found")
    else:
        log.info("Please select the Genesis 8 mesh or armature")
