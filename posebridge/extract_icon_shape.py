...
def extract_shape_from_mesh(obj):
    ...
    if obj.type != 'MESH':
        log.warning(f"ERROR: {obj.name} is not a mesh")
        return None
    ...
    if not vertices:
        log.warning("ERROR: No vertices found")
        return None
    ...
    if size == 0:
        log.warning("ERROR: Shape has zero size")
        return None
...
def extract_ordered_outline(obj):
    ...
    if len(bm.edges) == 0:
        log.warning("ERROR: No edges found")
        bm.free()
        return None
...
if __name__ == "__main__":
    ...
    if obj is None:
        log.warning("ERROR: No object selected")
    else:
        log.info("\n" + "="*60)
        log.info(f"EXTRACTING SHAPE FROM: {obj.name}")
        log.info("="*60)
        ...
        if vertices:
            ...
            log.info(format_for_icons_py(obj.name, vertices))
            log.info("\n--- END ---\n")
        else:
            # Fallback to simple extraction
            log.info("Could not find connected path, using vertex order...")
            ...
            if vertices:
                ...
                log.warning("ERROR: Could not extract vertices")
