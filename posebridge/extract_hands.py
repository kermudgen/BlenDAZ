...
def extract_hand_geometry(source_mesh_name, hand_side='left', hands_coll=None, char_tag=None):
    ...
    if source_mesh_name not in bpy.data.objects:
        log.warning(f"ERROR: Source mesh '{source_mesh_name}' not found")
        return None, None

    source_obj = bpy.data.objects[source_mesh_name]
    if source_obj.type != 'MESH':
        log.warning(f"ERROR: '{source_mesh_name}' is not a mesh")
        return None, None
    ...
    if not group_indices:
        log.warning(f"ERROR: No hand vertex groups found for {hand_side} hand")
        return None, None

    log.info(f"Found {len(group_indices)} vertex groups for {hand_side} hand")
    ...
    log.info(f"Found {len(hand_vert_indices)} vertices for {hand_side} hand")

    if len(hand_vert_indices) == 0:
        log.warning(f"ERROR: No vertices found for {hand_side} hand")
        return None, None
    ...
    log.info(f"Created hand mesh with {len(new_bm.verts)} verts, {len(new_bm.faces)} faces")
    ...
    log.debug(f"  Geometry center: {geometry_center}")
    ...
    log.debug(f"  Origin set to geometry center, shading smooth")

    return hand_obj, geometry_center


def position_hands_for_view(left_hand_obj, right_hand_obj, z_offset=-53.0, scale=1.0):
    """
    ...
    Transforms calibrated manually for Genesis 8 female (Fey, 1.23m) - 2026-02-17
    Positions scale proportionally to character height via ``scale`` factor.
    ...
    Args:
        ...
        scale: Character scale factor (char_height / 1.23). 1.0 = Fey reference.
    """

    if not left_hand_obj or not right_hand_obj:
        log.warning("ERROR: Missing hand objects")
        return False
    ...
    # X/Y positions and Z micro-offsets scale with character size;
    # rotations are size-independent.
    s = scale

    # Left hand: on left side of view, thumb pointing right
    left_hand_obj.location = Vector((-0.040003 * s, -0.68324 * s, z_offset + 0.022 * s))
    ...
    right_hand_obj.location = Vector((0.026067 * s, -0.67678 * s, z_offset - 0.049 * s))
    right_hand_obj.rotation_euler = (math.radians(187.13), math.radians(32.286), math.radians(95.809))

    log.debug(f"Positioned hands at Z≈{z_offset} (scale={s:.3f})")
    log.info(f"  Left hand: {left_hand_obj.location}")
    log.info(f"  Right hand: {right_hand_obj.location}")

    return True


def create_hand_camera(z_offset=-53.0, camera_distance=2.0, ortho_scale=0.7, hands_coll=None, char_tag=None, scale=1.0):
    """
    ...
    # Create camera — scale ortho_scale to frame larger/smaller hands
    camera_data = bpy.data.cameras.new(camera_name)
    camera_data.type = 'ORTHO'
    camera_data.ortho_scale = ortho_scale * scale

    camera = bpy.data.objects.new(camera_name, camera_data)
    target = hands_coll if hands_coll else bpy.context.scene.collection
    target.objects.link(camera)

    # Hands are positioned at Y ≈ -0.67 * scale, camera further back
    hands_y = -0.67 * scale
    camera.location = Vector((0, hands_y - camera_distance * scale, z_offset))
    camera.rotation_euler = (math.radians(90), 0, 0)  # Looking down +Y axis

    log.debug(f"Created hand camera at {camera.location} (scale={scale:.3f})")
    log.debug(f"  Ortho scale: {ortho_scale * scale:.3f}")
...
def get_transformed_bone_positions(armature_name, hand_obj, geometry_center, hand_side='left'):
    ...
    if armature_name not in bpy.data.objects:
        log.warning(f"ERROR: Armature '{armature_name}' not found")
        return {}

    armature = bpy.data.objects[armature_name]
    if armature.type != 'ARMATURE':
        log.warning(f"ERROR: '{armature_name}' is not an armature")
        return {}
    ...
    for bone_name in FINGER_BONES:
        ...
        if full_bone_name not in armature.pose.bones:
            log.warning(f"  Warning: Bone '{full_bone_name}' not found")
            continue
    ...
    log.debug(f"  Transformed {len(bone_positions)} positions for {hand_side} hand")
    return bone_positions


REF_HEIGHT = 1.23  # Fey's height — calibration reference for hand layout


def extract_and_setup_hands(standin_mesh_name, z_offset=-53.0, armature_name=None, char_name=None, char_tag=None, char_height=None):
    """
    ...
    Args:
        ...
        char_height: Character height in metres (used to scale layout). Defaults to REF_HEIGHT.
    ...
    # Scale factor relative to Fey (calibration reference)
    scale = char_height / REF_HEIGHT if char_height else 1.0

    log.info("\n" + "="*60)
    log.info("EXTRACTING HANDS FOR POSEBRIDGE HAND PANEL")
    log.debug(f"  char_height={char_height}, scale={scale:.3f}")
    log.info("="*60)
    ...
    log.info("\n--- Positioning hands ---")
    position_hands_for_view(left_hand, right_hand, z_offset=z_offset, scale=scale)

    # Create camera
    log.info("\n--- Creating hand camera ---")
    camera = create_hand_camera(z_offset=z_offset, hands_coll=hands_coll, char_tag=char_tag, scale=scale)
    ...
    log.info("\n" + "="*60)
    log.info("HAND EXTRACTION COMPLETE")
    log.info("="*60)
    log.info(f"\nCreated objects:")
    log.info(f"  - {left_hand.name}")
    log.info(f"  - {right_hand.name}")
    log.info(f"  - {camera.name}")
    if armature_name:
...
def store_hand_control_points(result):
    ...
    log.info(f"Stored {len(all_hand_cps)} hand control points")
    ...
    if standin_name in bpy.data.objects:
        ...
        if result and result.get('left_bone_positions'):
            ...
            for bone, pos in result['left_bone_positions'].items():
                log.debug(f"  {bone}: ({pos.x:.4f}, {pos.y:.4f}, {pos.z:.4f})")
    else:
        log.warning("ERROR: Could not find standin mesh. Available meshes:")
