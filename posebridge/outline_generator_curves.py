...
def create_genesis8_body_outline(mesh_obj, outline_name="PB_Outline_Body"):
    ...
    print("Detecting silhouette edges from front view...")
    ...
    if not stroke_chains:
        print("No stroke chains generated")
        return None
    ...
    print(f"Created outline: {outline_name} with {len(stroke_chains)} splines")
    return curve_obj
...
if __name__ == "__main__":
    ...
    if mesh_obj:
        if mesh_obj.type == 'MESH':
            print(f"Generating outline for mesh: {mesh_obj.name}")
            create_genesis8_body_outline(mesh_obj)
        elif mesh_obj.type == 'ARMATURE':
            # Find mesh that uses this armature
            print(f"Finding mesh for armature: {mesh_obj.name}")
            for scene_obj in bpy.context.scene.objects:
                if scene_obj.type == 'MESH':
                    for mod in scene_obj.modifiers:
                        if mod.type == 'ARMATURE' and mod.object == mesh_obj:
                            print(f"Found mesh: {scene_obj.name}")
                            create_genesis8_body_outline(scene_obj)
                            break
        else:
            print(f"Selected object is {mesh_obj.type}, need MESH or ARMATURE")
    else:
        print("No object selected - please select Genesis 8 mesh or armature")
