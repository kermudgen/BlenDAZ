...
class VIEW3D_PT_poseblend_main(Panel):
    ...
    def draw(self, context):
        ...
        if settings.is_active:
            row.operator("poseblend.deactivate", text="Exit PoseBlend", icon='CANCEL')
        else:
            row.operator("poseblend.activate", text="Enter PoseBlend", icon='POSE_HLT')
        ...
        row = layout.row()
        row.scale_y = 1.5
        row.operator("poseblend.interact", text="Start Blending", icon='HAND')
