...
### Recent Implementation: Mesh Mannequin as Character Stand-in

**Date:** 2026-02-19

**What was implemented:**
Instead of hiding the mesh copy after outline generation, it is now kept visible in the viewport as a gray mannequin background for the control node dots.

**Changes in `outline_generator_lineart.py` (cleanup section):**
...
Now the script:
1. **Unlocks all transform channels** — DAZ meshes have locked loc/rot/scale; these are freed on the copy
2. **Strips all modifiers** — removes armature, subdivision, etc. from the copy
3. **Clears all materials** — removes DAZ skin/texture materials
4. **Applies `PB_Mannequin_Gray` material** — flat gray (0.35 value), full roughness, zero specular; material is reused across runs to avoid duplicates
5. **Keeps visible in viewport** (`hide_viewport = False`), hidden from render
