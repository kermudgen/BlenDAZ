...
def _find_geografts(context, armature):
    ...
    if selected_meshes:
        print(f"[BlenDAZ Init] Found {len(selected_meshes)} geograft(s) from selection: "
    ...
    if grafts:
        print(f"[BlenDAZ Init] Found {len(grafts)} geograft(s) via scene scan: "
        ...
        for obj in context.selected_objects:
            if obj.type == 'MESH' and obj is not body_mesh:
                ...
                print(f"[BlenDAZ Init] Selected mesh '{obj.name}' — "
...
def _run_face_group_remap(context, live_mesh_obj, reference_mesh_name, armature):
    ...
    Returns:
        (ok: bool, message: str)
    """
    # Ensure dsf_face_groups is importable
    ...
    print(f"  Reference mesh: {reference_mesh_obj.name} ({len(reference_mesh_obj.data.polygons)} polygons)")
    print(f"  Live mesh:      {live_mesh_obj.name} ({len(live_mesh_obj.data.polygons)} polygons)")
    ...
    try:
        ...
        if live_instance is not None:
            ...
            print("  [BlenDAZ Init] Updated live operator instance with remapped FaceGroupManager")
        else:
            print("  [BlenDAZ Init] Cleared operator caches (modal not running — will apply on next start)")
    except Exception as e:
        print(f"  [BlenDAZ Init] Could not update operator caches (non-fatal): {e}")
...
after geograft merge. Run this BEFORE merging geografts."""
    ...
    def execute(self, context):
        ...
        if existing:
            print(f"[BlenDAZ Init] Mannequin copy already exists: {existing.name}")
        ...
        try:
            ...
            if posebridge_dir not in sys.path:
                sys.path.insert(0, posebridge_dir)

            import outline_generator_lineart
            import importlib
        ...
        print(f"[BlenDAZ Init] Snapshot complete. Reference mesh: {copy_obj.name}")
        return {'FINISHED'}
        ...
        if not grafts:
            ...
            try:
                def _do_merge():
                    ...
                    for g in grafts:
                        g.select_set(True)
                    print(f"[BlenDAZ Init] Merging {len(grafts)} geograft(s) into '{live_mesh.name}'")
                ...
                print("[BlenDAZ Init] Geograft merge complete")
            except Exception as e:
                import traceback
                traceback.print_exc()
