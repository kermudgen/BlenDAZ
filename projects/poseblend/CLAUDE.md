...
## Project Structure
```
D:\Dev\BlenDAZ\projects\poseblend\
...
- DAZ Genesis 8 rigs via Diffeomorphic import (primary target rig)
...
### After Code Changes
```python
# Reload all modules (no full restart needed)
import importlib
import poseblend
importlib.reload(poseblend.core)
importlib.reload(poseblend.blending)
# etc.
```

## Current Focus
- All modules scaffolded; not yet tested in Blender
- **First fix**: `default_mask_mode` crash bug (see Known Bugs)
- **Second fix**: `draw_label()` with `blf` so dot names render
- Phase 1 infrastructure all present; Phase 2+ partially written but untested
